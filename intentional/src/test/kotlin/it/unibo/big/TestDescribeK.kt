...
class TestDescribeK {
    ...
    fun `test covid countries`() {
        ...
        d = DescribeExecute.parse("with COVID-19 describe deaths by month, countriesAndTerritories for countriesAndTerritories in ('United_States_of_America', 'Italy') and month > '2020-10'", false)
        cube = DescribeExecute.execute(d, path, oldInterest = false).second
        cube = cube.sortedBy("month", "countriesandterritories")
        c = dataFrameOf("deaths", "month", "countriesandterritories", "novelty", "surprise", "zscore_deaths", "peculiarity")(
                16583,	"2020-11",	"Italy",	                1.000,	1.000,	-0.558, 0.425,
                37165,	"2020-11",	"United_States_of_America",	1.000,	1.000,	 1.313, 1.000,
                 8483,	"2020-12",	"Italy",	                1.000,	1.000,	-1.294, 0.986,
                28649,	"2020-12",	"United_States_of_America",	1.000,	1.000,	 0.539, 0.411,

        )
        equals(c, cube)

        d = DescribeExecute.parse(d, "with COVID-19 describe deaths by month, countriesAndTerritories for countriesAndTerritories in ('United_States_of_America', 'Italy', 'Spain') and month > '2020-10'", false)
        cube = DescribeExecute.execute(d, path, oldInterest = false).second
        cube = cube.sortedBy("month", "countriesandterritories")
        c = dataFrameOf("deaths", "month", "countriesandterritories", "novelty", "surprise", "peculiarity")(
                16583, "2020-11", "Italy",                    0.0, 0.0, 0.430,
                 9191, "2020-11", "Spain",                    1.0, 0.5, 0.544,
                37165, "2020-11", "United_States_of_America", 0.0, 0.0, 0.283,
                 8483, "2020-12", "Italy",                    0.0, 0.0, 0.487,
                 2555, "2020-12", "Spain",                    1.0, 0.5, 1.000,
                28649, "2020-12", "United_States_of_America", 0.0, 0.0, 0.344,
        )
        equals(c, cube)
    }
    ...
    fun `test covid`() {
        try {
            ...
            d = DescribeExecute.parse("with COVID-19 describe deaths by countriesAndTerritories, month")
            c = DescribeExecute.execute(d, path).second

            d = DescribeExecute.parse("with COVID-19 describe deaths by month for countriesAndTerritories = 'United_States_of_America'")
            c = DescribeExecute.execute(d, path).second
            assertFalse(c.names.contains("model_skyline"), c.names.toString())
            d = DescribeExecute.parse(d, "with COVID-19 describe deaths, cases by month, countriesAndTerritories for countriesAndTerritories = 'United_States_of_America'", false)
            ...
            d = DescribeExecute.parse("with covid describe cases by countriesandterritories", false)
            DescribeExecute.execute(d, path)
